...
/**
 * This is a last-resort endpoint that bypasses ALL middleware and security checks.
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Campaign created successfully via bypass endpoint',
      campaignId,
      slug
    }, { status: 201 });
  } catch (error) {
    ...
    return NextResponse.json({
      success: false,
      message: 'Failed to create campaign with bypass endpoint',
